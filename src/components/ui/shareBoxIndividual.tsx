...
export function ShareBoxIndividual({open, onClose,contentId, contentIdSetfn}: propTypes) {
    ...
    const [selectType, setSelectType] = useState<setContentType | null>();
    ...
    console.log("true or false ",selectType === setContentType.Yes);
    async function shareLink(ifTrue: boolean) {

        const response = await axios.post(`${BACKEND_URL}/api/v1/content/shareone/${contentId}`,{
            shareMemory: ifTrue,
            contentId: contentId
        },{
            headers: {
        ...
        const data = await response.data as {individualSharableLink : string, hash: string}
        console.log(data);
    ...
    useEffect(() => {
        if (selectType === setContentType.Yes) {
            shareLink(true); 
        } else if (selectType === setContentType.No) {
            shareLink(false)
        }
    }, [selectType]);
                    ...
                    <div className="flex items-center justify-center h-full">
                        <div className="w-80 sm:w-96 p-7 shadow-md border rounded-xl max-h-min bg-white h-2/5">
                            ...
                            <div className="pt-6 flex gap-5 justify-start">         
                                <span onClick={contentIdSetfn}><InputCheckbox checkedd={selectType === setContentType.Yes} name="Yes" selectType={() => selectTypefn(setContentType.Yes)}  key={1} /></span>
                                <InputCheckbox checkedd={selectType === setContentType.No} name="No" selectType={() => selectTypefn(setContentType.No)}  key={2} />
                            </div>
